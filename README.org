...
- [[#virtual-columns-as-computed-bins][Virtual columns as computed bins]]
- [[#chaining][Chaining]]
- [[#tricks][Tricks]]
...
:PROPERTIES:
:CUSTOM_ID: virtual-columns-as-computed-bins
:END:
...
One way is to add a =Month= column to the input table. The new table
looks like:
...
If the title =Month= is not specified, then the new virtual column can
be referred to as =$3=.

Note that here the title was specified with single quotes. This is not
required for simple alphanumeric names. Single quotes are useful for
instance if the title includes a blank character.

The precompute may also be Lisp expressions, exactly like in the
...
formulas. Or they may be used by the optional row filter. There is no
difference between actual and virtual columns.
