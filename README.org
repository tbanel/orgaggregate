...
* New
The wizard (see [[#key-binding--wizard][Key-binding & Wizard]]) has been extended. It can now
...
Still call it with =C-c C-x x aggregate=.
...
tables. Tables may lie in distant files as well in the current
buffer. They may be computed on the fly by Babel blocks as well as
regular Org Mode tables. They may also be pointed to by Org
IDs.

Regular Org Mode table slicing is now recognized. Pre & post
processors are now handled by the wizard, as well as the hline
parameter.
