...
* New
JSON and CSV input tables can now be inside Org Mode blocks.
(Previously they could live only in their own separate file.).

See [[#csv-input][CSV input]] and [[#json-input][JSON input]].
...
- [[#stop-reading-here-8020][Stop reading here! 80/20]]
  ...
  - [[#dont-try-to-do-what-orgaggregate-was-not-designed-for][Don't try to do what OrgAggregate was not designed for]]
- [[#equivalent-in-sql-r-datamash][Equivalent in SQL, R, Datamash]]
...
** To summarize the 20%
:PROPERTIES:
:CUSTOM_ID: to-summarize
:END:

- *Data:* select one of your Org table, and be ready to aggregate values
  from it right in the same file.

- *Name:* give a name to your table with a special line just above it.
...
- *Block:* create an aggregation block.
...
- *Refresh:* type =C-c C-c= on the =#+begin:= line now and whenever you
   want to refresh the aggregation.
...
** Keep it simple
:PROPERTIES:
:CUSTOM_ID: dont-try-to-do-what-orgaggregate-was-not-designed-for
:END:

She (the user) may be tempted to add the grand total at the bottom of
the aggregation. Don't do that!

Example of such a temptation:
...
Even though OrgAggregate offers ways to do that (for instance with the
=:post= parameter, see [[#algorithm-post-processing][Algorithm post processing]]), there are many
reasons not to. If she does, she is quietly entering the same
nightmare which plagues spreadsheets. Everything will become harder
...
#+begin_example
,#+begin: aggregate :table original :cols "vmean(Level) vsum(Quantity)"
...
* Equivalent in SQL, R, Datamash
:PROPERTIES:
:CUSTOM_ID: equivalent-in-sql-r-datamash
:END:
