...
The block may contain additional information beside the old
table. Example:

#+begin_example
#+begin: aggregate :table original :cols "Day vsum(Quantity)"
note: my table should include the "Year" column.
#+name: my_aggregated-table
| Day     | vsum(Quantity) |      |
|---------+----------------+------|
| Monday  |             14 | 1400 |
| Tuesday |             45 | 4500 |
#+tblfm: $3=$2*100
#+plot: …
Note: change the style of the plot.
#+end:
#+end_example

In this example, the block contains tags like =#+name:= and free text
like "note: my table…". All that is preserved upon refresh (=C-c C-c=)
or wizard update (=C-c C-x x=) (see [[#guiding-traditional-wizard][Guiding (traditional) wizard]]). Blank
lines are removed thought.

The =#+tblfm:= tag is special. It is preserved of course. It may also be
enriched by a possible =:formula= parameter (see [[#spreadsheet-formulas][Spreadsheet
formulas]]). The new table computed by aggregation is then updated by
...
OrgAggregate is great when you want to output an Org Mode
...
(require 'tblfn)
(tblfn-join
 (tblfn-aggregate original "Day" "Level" "vmean")
 (tblfn-aggregate original "Day" "Quantity")
 "Day")
,#+end_src
#+end_example

Two separate aggregations are performed, then they are joined
together. As the =el-tblfn= package is quite young, this is subject to
...
re-computations. And it may be enriched by a =:formula= parameter as in
the previous example.

The spreadsheet application is delegated to Org Mode, ensuring that
everything works reliably. However many side processings done by Org
Mode are bypassed, mainly because the table handled to Org Mode is a
fresh one. The alignment of columns is done by OrgAggregate, which is
way faster than the Org Mode one. The result is about 5× faster than
the stock Org Mode one.
...
- Tilmann Singer, computed aggregating bins, ="month(Date)"= in his use
  case
- Misohena (on GitHub this time) gave feedback on the =el-tblfn=
  example.
