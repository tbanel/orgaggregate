...
or in the form of $1, $2, ..., or the virtual column hline) are key
columns.  Everything else is given to Calc, to be computed as an
aggregation.

For instance:
#+BEGIN_EXAMPLE
(3)                      ;; a constant
mean(2*X+1)              ;; aggregate a complex expression
exp(mean(map(log,N)))    ;; the exponential average
sum((X-mean(X))^2)       ;; X-mean(X) centers the sample on zero
#+END_EXAMPLE

Arguably, the first expression is useless.
The aggregation can be applied on a computed list of values.
The result of an aggregation can be further processed in a formula.
An aggreagation can even be applied on an expression containing another aggregation.
...
The special expression =(C)= (a column name withing parenthesis)
yields a list of values from this column to be aggregated, except they
...
An expression may optionally be followed by modifiers and formatter, after a semicolon. Examples:

#+BEGIN_EXAMPLE
sum(X);p20    ;; increase Calc internal precision to 20 digits
sum(X);f3     ;; output the result with 3 digits after the decimal dot
sum(X);%.3f   ;; output the result with 3 digits after the decimal dot
#+END_EXAMPLE
