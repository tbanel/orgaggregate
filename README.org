...
converted to zero, depending on modifier flags =E= and =N=. This topic
is currently experimental, and may change.

The empty cells treatment
- makes no difference for =sum= and =count=.
- may result in zero for =prod=,
- change =mean= result,
- change =min= and =max=, a possibly empty list of values resulting in
  =inf= or =-inf=
