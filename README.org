...
This is the third version, allowing non necessarilly alphanumeric
...
Column names are not necessarilly alphanumeric words. They can contain
any characters, including spaces, quotes, +, -, whatever.
...
Note that in =:cond= lisp expression, only double quotes work.
...
- Alejandro Erickson, non-alphanumeric column names
