...
New: sort output, hlines on output.
...
In the latest releases of Org Mode (9.4), there is a similar wizard
(thanks to Kin Stardiviner). Call it with =C-c C-x x=.
...
In this previous example, output hline are unrelated to input ones.
